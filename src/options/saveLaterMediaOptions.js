...
class SaveLaterMediaOptions extends React.Component {
    state = {
        laterSlug: '',
        captionTemplateMap: {},
        templates: [],
    ...
    setSelectedTemplate = (selectedTemplate) => {
        ...
        const [ newCaptionTemplate, hashtagList ] = captionTemplateMap[selectedTemplate];
        const hashtagListString = hashtagList.join('\n')
        this.setState({ selectedTemplate, newCaptionTemplate,  hashtagListString });
    }
    ...
    setHashtagListString = hashtagListString => {
        this.setState({ hashtagListString });
    }
    ...
    componentDidMount() {
        chrome.storage.local.get(['captionTemplateMap', 'hashtagListMap', 'hashtagGroups', 'laterSlug'], (result) => {
    ...
    handleInitializeLater = async () => {
        ...
        if (response.status === 200) {
            ...
            chrome.storage.local.set({ laterSlug: mainGroupSlug, laterLabelMap: labelMap, igNameToProfileId }, () => {
    ...
    handleSaveTemplate = () => {
        const { newCaptionTemplate, captionTemplateMap, selectedTemplate, hashtagListString } = this.state;

        const newMap = { ...captionTemplateMap, [selectedTemplate]: [newCaptionTemplate, hashtagListString.split('\n')] };
        chrome.storage.local.set({ captionTemplateMap: newMap }, () => {
            this.setCaptionTemplateMap(newMap);
        });
    }
    ...
    render() {
        ...
        const [ captionTemplate, hashtagList ] = captionTemplateMap[selectedTemplate] || ['', []];
        ...
        const hasHashtagListChanged = hashtagListString !== hashtagList.join('\n');
        const isNewCaptionTemplateSaveable = hasValidTemplateNameSelected && (hasCaptionTemplateChanged || hasHashtagListChanged);
                ...
                <div class='container mt-5'>
                    ...
                    <div class='row justify-content-center'>
                        <div class='col-6'>
                            ...
                            <div class='form-group'>
                                <label for='caption-template-input'>Edit Hashtag List</label>
                                <label for='caption-template-input'>A list of hashtags you want to use with this template (only ~25 will )</label>

                                <textarea
                                    disabled={!hasValidTemplateNameSelected}
                                    class='form-control'
                                    rows='10'
                                    value={hashtagListString}
                                    onChange={e => this.setHashtagListString(e.target.value)}
                                />
                            </div>
                            <button
...
export default SaveLaterMediaOptions;
